    ...
    <div className="space-y-8 max-w-7xl mx-auto py-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Manage, analyze, and query your uploaded contracts and general documents.</p>
        </div>
        ...
        <label className="cursor-pointer bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-xl font-bold text-xs transition-all shadow-lg shadow-indigo-600/20 flex items-center justify-center space-x-2">
          {uploading ? (
            <>
              ...
              <span>Categorizing & Analyzing Document...</span>
            </>
          ) : (
            <>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-white flex items-center space-x-2">
          ...
          <span>Ingested Documents & Sample Library</span>
        </h2>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-white ${
                        isLegal ? 'bg-indigo-600/20 border border-indigo-500/30 text-indigo-400' : 'bg-amber-600/20 border border-amber-500/30 text-amber-400'
                      }`}>
                        <FileText className="w-5 h-5" />
                      </div>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${
                        isLegal ? 'bg-indigo-500/10 text-indigo-300 border-indigo-500/20' : 'bg-amber-500/10 text-amber-300 border-amber-500/20'
                      }`}>
                        {docType}
                      </span>
                    </div>
